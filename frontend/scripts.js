...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("createRoomForm").addEventListener("submit", async (e) => {
    ...
    try{
      const response = await fetch(`${location.protocol}//mafiavoteout-backend2.onrender.com/api/v1/room/`,
        {
          method: "POST",
  ...
  document.getElementById('joinRoomForm').addEventListener('submit', async (e) => {
    ...
    try{
      const response = await fetch(`${location.protocol}//mafiavoteout-backend2.onrender.com/api/v1/player/`,
        {
          method: "POST",
          headers: {'Content-Type':'application/json'},
          body: JSON.stringify(data)
